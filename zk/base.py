...
class ZK(object):
    ...
    ##### For using V5L Machine, Author: Ibrahim Khalil Ullah, Contributor: Farhan Afsar########
    def get_attendance(self):
        ...
        users = self.get_users()
        uid_to_user_id = {u.uid: u.user_id for u in users}
        user_id_to_uid = {u.user_id: u.uid for u in users}

        attendances = []
        attendance_data, size = self.read_with_buffer(const.CMD_ATTLOG_RRQ)

        if size < 4:
            print("No attendance data from device.")
            return []
        ...
        print(f"DEBUG: Total Records: {self.records}, Total Size: {total_size}, Record Size: {record_size}")
        print(f"DEBUG: Attendance Data Length: {len(attendance_data)}")

        while len(attendance_data) >= record_size:
            try:
                chunk = attendance_data[:record_size]
                print(f"DEBUG: Processing chunk of size {len(chunk)}")

                if record_size == 8:
                    uid, status, timestamp, punch = unpack('HB4sB', chunk)
                    user_id = uid_to_user_id.get(uid, str(uid))
                    timestamp = self.__decode_time(timestamp)
                    attendances.append(Attendance(user_id, timestamp, status, punch, uid))
                    attendance_data = attendance_data[8:]

                elif record_size == 16:
                    user_id_raw, timestamp, status, punch, _, _ = unpack('<I4sBB2sI', chunk)
                    user_id = str(user_id_raw)
                    uid = user_id_to_uid.get(user_id, user_id_raw)
                    timestamp = self.__decode_time(timestamp)
                    attendances.append(Attendance(user_id, timestamp, status, punch, uid))
                    ...
                    if len(chunk) < 49:
                        print("DEBUG: Skipping incomplete record")
                        break

                    uid, raw_user_id, status, timestamp, punch, _ = unpack('<H24sB4sB17s', chunk)
                    user_id = raw_user_id.split(b'\x00')[0].decode(errors='ignore')
                    timestamp = self.__decode_time(timestamp)
                    attendances.append(Attendance(user_id, timestamp, status, punch, uid))
                    ...
                    print(f"Unknown record size: {record_size}")
                    break
        ...
        print(f"DEBUG: Parsed {len(attendances)} records")
        return attendances
